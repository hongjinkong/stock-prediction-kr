...
REM    사람이 검토 후 직접 `python paper_trade.py --execute` 한다.
REM  - 완전 자동매매로 가려면 아래 advisory 줄을
REM    `python paper_trade.py --execute --notify` 로 바꾸면 된다(자기 책임).
REM ============================================================
cd /d "C:\Users\smhrd\Desktop\주식예측모델"

echo [%date% %time%] 자문 리포트 실행 >> "reports\cron.log"
python advisory.py --notify  >> "reports\cron.log" 2>&1
python monitor.py  --notify  >> "reports\cron.log" 2>&1
